...
template <class STR>
xercesc::KVStringPair* KVStringPair_fromstring(const STR& key, const STR& value, xercesc::MemoryManager* const manager) {
	XMLString buff1(key), buff2(value);
	return new xercesc::KVStringPair(buff1.ptr(), buff2.ptr(), manager);
}

template <class STR>
xercesc::KVStringPair* KVStringPair_fromstring(const STR& key, const STR& value) {
...
void KVStringPair_init(void) {
	...
	boost::python::class_<xercesc::KVStringPair, boost::python::bases<xercesc::XSerializable> >("KVStringPair", boost::python::init<boost::python::optional<xercesc::MemoryManager* const> >())
			...
			.def("__init__", boost::python::make_constructor(static_cast<xercesc::KVStringPair*(*)(const XMLString&, const XMLString&, xercesc::MemoryManager* const)>(&KVStringPair_fromstring<XMLString>)))
			.def("__init__", boost::python::make_constructor(static_cast<xercesc::KVStringPair*(*)(const XMLString&, const XMLString&)>(&KVStringPair_fromstring<XMLString>)))
			.def("__init__", boost::python::make_constructor(static_cast<xercesc::KVStringPair*(*)(const std::string&, const std::string&, xercesc::MemoryManager* const)>(&KVStringPair_fromstring<std::string>)))
			.def("__init__", boost::python::make_constructor(static_cast<xercesc::KVStringPair*(*)(const std::string&, const std::string&)>(&KVStringPair_fromstring<std::string>)))
			.def(KVStringPairDefVisitor<XMLString>())
			.def(KVStringPairDefVisitor<std::string>())
